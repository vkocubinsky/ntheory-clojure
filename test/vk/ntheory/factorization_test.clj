...
(deftest odd-table-content-test
  (are [upper-limit numbers] (= numbers (let [factorization (f/make-factorization {:type :odd-table :upper-limit upper-limit})
...
(deftest odd-sieve-factorization-test
  (let [factorizer (f/make-factorization {:type :odd-table :upper-limit helpers/factorization-data-upper-limit})]
...
(deftest odd-trial-division-factorization-test
  (let [factorizer (f/make-factorization {:type :odd-trial-division :cache-upper-limit helpers/factorization-data-upper-limit})]
    (helpers/factorization-test-helper factorizer)
    ))


(deftest odd-trial-division-primes-test
  (let [factorizer (f/make-factorization {:type :odd-trial-division :cache-upper-limit 11})]
